...
const ProductCard = (props) => {
  ...
  return(
    <div className={styles.product__container}>
      <img src={product.url} alt="Trulli" className={styles.product__image}/>
      <div className={styles.product__detail__container}>
        <label className={styles.product__name__text}>{product.name}</label>
        <label className={styles.product__detail__text}>Rating: {product.rating}</label>
        <label className={styles.product__detail__text}>Timer: {pad2(minuts)}:{seconds}</label>
        <button className={styles.product__detail__button}>Detalle</button>
      </div>
    </div>
  )
}
